		...
		<div id="main_view" class="main_view" >
			<table cellSpacing=0 width="100%" class="content_view">
				<tr>
					...
					<td ><input type="text" name="title" id="title" class="validate input-text" size="40" validtip="required"  value="<?=isset($view['title'])?$view['title']:'';?>"></td>
				...
				<tr>
					<td width="100"><?=lang('type_pselect')?></td>
					<td>
						<select name="type" id="type" class="validate"  validtip="required">
							<option value=""><?=lang('type_pselect')?></option>
							<?php foreach($typearr as $type):?>
				...
				<tr>
					<td width="100"><?=lang('url')?></td>
					<td><input type="text" name="url" id="url" class="validate input-text"  validtip="required"  size="40" value="<?=isset($view['url'])?$view['url']:'';?>"></td>
				</tr>
			<?php if($view['type']==-1):?>
				<tr>
					<td width="100"><?=lang('linktype')?></td>
					<td>
  			  			<select name="linktype" id="linktype">
    						 <option value="1" <?php if($view['linktype']==1){echo "selected";}?>><?=lang('linktype1')?></option>
  							 <option value="2" <?php if($view['linktype']==2){echo "selected";}?>><?=lang('linktype2')?></option>
   						 </select>
    				</td>
    			</tr>    
				<tr>
					<td width="100"><?=lang('username')?></td>
					<td><input type="text" name="username" id="username" class="input-text"  size="40" value="<?=isset($view['username'])?$view['username']:'';?>"></td>
				</tr>
   				<tr>
   					<td width="100"><?=lang('tel')?></td>
   					<td><input type="text" name="tel" id="tel" class="input-text"  size="40" value="<?=isset($view['tel'])?$view['tel']:'';?>"></td>
   				</tr>
   				<tr>
   					<td width="100"><?=lang('myemail')?></td>
   					<td><input type="text" name="email" id="email" class="input-text"  size="40" value="<?=isset($view['email'])?$view['email']:'';?>"></td>
   				</tr>
   				<tr>
   					<td width="100"><?=lang('qq')?></td>
   					<td><input type="text" name="qq" id="qq" class="input-text"  size="40" value="<?=isset($view['qq'])?$view['qq']:'';?>"></td>
   				</tr>
  			 <?php endif;?>

				<tr>
					<td width="100"><?=lang('description')?></td>
					<td><input type="text" name="description" id="description" class="input-text" size="40" value="<?=isset($view['description'])?$view['description']:'';?>"></td>
				</tr>
				<tr>
					<td width="100"><?=lang('remark')?></td>
					<td colspan="2"><textarea rows="3" cols="40" name="remark" id="remark" class="txtarea"><?=isset($view['remark'])?$view['remark']:'';?></textarea></td>
				</tr>
				<tr>
					<td width="100"><?=lang('status')?></td>
					<td colspan="2"><?=lang('status1')?>
						<input type="radio" name="status" value="1" <?php if(!isset($view['status'])||$view['status']==1){echo 'checked';} ?> /><?=lang('status0')?>
						<input type="radio" name="status" value="0" <?php if(isset($view['status'])&&$view['status']==0){echo 'checked';} ?>  />
					</td>
				</tr>
				<tr>
					<td width="100"><?=lang('order')?></td>
					<td colspan="2"><input type="text" name="listorder" id="listorder" value="<?php if(isset($view['listorder'])){echo $view['listorder'];}else{echo '99';} ?>" class="input-text" value=""></td>
				</tr>
			</table>
		</div>
	</form>
