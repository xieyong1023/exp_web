...
class Main extends CI_Controller {
	 ...
	 * username, 用户名
	 * varname, 
	 * $list, 顶部的导航框列表信息(主要是每个按钮的id和class)
	 * $editlang, zh_cn
	 * $langlist, zh_cn
	 */
	...
	/*
	 * 此处加载的内容有:
	 * $purview, 所有具有权限的模块(所有左侧的导航框信息)这个数组可以var_dump()看一下
	 * $parent, 这个值=32
	 */
	...
	public function attrlist(){
		...
		if ($path_name=='') {
			...
			$moveup_dir_path = preg_replace('/(.*?)[^\/]+\/$/', '$1', $current_dir_path);//只读取GET到的路径的第一段
		}
		...
		foreach($listarr as $filename){
			$filepath = $current_path.$filename;  //每一个请求目录下的文件的路径
			$file = get_file_info($filepath);     //获得文件信息
			if(is_dir($filepath)){
				$tmpDir =directory_map($file['server_path'],1);  //下一级目录
