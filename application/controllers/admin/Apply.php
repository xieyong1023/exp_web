...
class Apply extends CI_Controller {
	var $tablefunc = 'apply';
	var $fields = array('category','jobs','name','sex','birth','marriage','height','address','eduresume','workresume','postcode','tel','phone','email','listorder','status');
	...
	function _setlist($data,$ismultiple=true){
		...
		foreach($newdata as $key=>$item){
			...
			<td>'.$categorystr.$item['jobs'].'</td>
			<td width=80>'.$item['name'].'</td>
			<td width=80>'.$item['sex'].'</td>
