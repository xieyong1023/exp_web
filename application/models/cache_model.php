...
class Cache_model extends CI_Model{
	...
	function setLang($get=array()){
        ...
        if($this->config->item('url_suffix')!='')
        {
            ...
            $get['lang']=str_replace($url_suffix,'',$get['lang']);
        }
	...
	function loadNavigation($type){
		...
		$cache = $this->CI->cache->get($cachestr);
		if(!$cache){
			$data = $this->CI->Data_model->getData(array('lang'=>$this->currentLang,'type'=>$type,'status'=>1),'listorder',0,0,'navigation');
			$num = 0;
			foreach($data as $item){
	...
	function loadCategoryByDir($dir){
		...
		if(!$cache){
			...
			$this->CI->cache->save($cachestr,$cache,2592000); //有效期1个月
		}
		return $cache;
	}
		...
		if(!$cache){
			...
			switch($order){
				...
				case 'rand':
					...
					break;
				default:
					$orderstr = 'listorder,puttime desc';
					break;
			}
	...
	function loadLink($type=0,$id=0){
		$cachestr = 'link_'.$this->currentLang.'_'.$type;
		if($id==0) $cache = $this->CI->cache->get($cachestr);
		if(!$cache){
	...
	function loadOnline($type=''){
		$cachestr = 'online_'.$this->currentLang;
		$cache = $this->CI->cache->get($cachestr);
		//if(!$cache){
            if($type!='')
            {
                $cache = $this->CI->Data_model->getData(array('lang'=>$this->currentLang,'status'=>1,'type'=>$type),'listorder,id desc',0,0,'online');
            }
            else
            {
	...
	function loadKeywords(){
		...
		if(!$data){
			...
			foreach($tmpdata as $item){
				$keywords[] = $item['title'];
				$urls[] = "<a href='".$item['url']."' target='_blank' title='".$item['title']."'>".$item['title']."</a>";
			}
	...
	********************************************************************************/
	...
	function loadMenu(){
		...
		if($cache === false){
			...
			$menuData = $this->db->get('category')->result_array();
			$cache = array();
			$cache['default'] = $menuData[0]['dir'];
			foreach ($menuData as $key => $item){
				$cache['top'][$item['dir']] = $item;
				$cache['id'][$item['id']] = array(
					'name' => $item['name'],
					'model' => $item['model'],
					'dir' => $item['dir'],
				);
				...
				$this->db->order_by('listorder', 'asc');
				$subMenuData = $this->db->get('category')->result_array();
				foreach ($subMenuData as $subMenu){
					$cache['id'][$subMenu['id']] = array(
						'name' => $subMenu['name'],
						'model' => $subMenu['model'],
						'dir' => $subMenu['dir'],
					);
					$cache['tree'][$item['dir']][$subMenu['dir']] = $subMenu;
				}
			}
			$this->CI->cache->save($cachestr,$cache,2592000);
            $this->db->close();
